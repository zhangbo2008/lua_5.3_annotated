...
//保护的进行代码运行. 运行函数f, 参数是ud
int luaD_rawrunprotected(lua_State *L, Pfunc f, void *ud)
{
    unsigned short oldnCcalls = L->nCcalls; //为什么要保存这个东西????来计算层数.后面再分析这个东西.
...
static int stackinuse(lua_State *L) //查询多少个已经使用过的
{
    CallInfo *ci; // ci是L->stack的一个子集.
    StkId lim = L->top;
    for (ci = L->ci; ci != NULL; ci = ci->previous) //因为是已经使用过的.所以要使用previous往前找.看看多少个.
    {
        if (lim < ci->top)
...
int luaD_poscall(lua_State *L, CallInfo *ci, StkId firstResult, int nres)
{
    ...
    L->ci = ci->previous; /* back to caller */ //返回上一个ci对象.
...
*/   //如果是c函数那么进行处理,否则使用luaV_execute进行处理.
int luaD_precall(lua_State *L, StkId func, int nresults)
{
    ...
    switch (ttype(func))
    {
    case LUA_TCCL: /* C closure */   //嵌套型函数
        f = clCvalue(func)->f;
        goto Cfunc;
    case LUA_TLCF: /* light C function */  //单纯的c函数
        f = fvalue(func);
    Cfunc:
    {
        int n;                                          /* number of returns */
        checkstackp(L, LUA_MINSTACK, func);             /* ensure minimum stack size */
        ci = next_ci(L); /* now 'enter' new function */ //创建一个新的ci
