...
static int newupvalue (FuncState *fs, TString *name, expdesc *v) {
  ...
  luaM_growvector(fs->ls->L, f->upvalues, fs->nups, f->sizeupvalues,
                  Upvaldesc, MAXUPVAL, "upvalues");  //  nups表示当前有多少个, sizeupvalues表示容量多大. Upvaldesc是upvalue_description表示一个upvalue的结构体. 拓展之后 f->sizeupvalues就表示新的数值了.
...
** so it is handled in separate. 判断这个token是不是一个语法块的结尾.
*/
...
static BinOpr subexpr (LexState *ls, expdesc *v, int limit) {
  ...
  else simpleexp(ls, v);//那么进入简单操作符.,处理完结果的写入v里面.
  /* expand while operators have priorities higher than 'limit' */
  op = getbinopr(ls->t.token);//找到二元运算.
...
//expr解析一个元素. subexpr是他的更底层实现. 前面加一个sub表示底层.
static void expr (LexState *ls, expdesc *v) {
  subexpr(ls, v, 0);
}
...
LClosure *luaY_parser (lua_State *L, ZIO *z, Mbuffer *buff,
  ...
  sethvalue(L, L->top, lexstate.h);  /* anchor it */// 站上放入一个字典.
  luaD_inctop(L);
