...
static int pushline(lua_State *L, int firstline)
{
  ...
  int readstatus = lua_readline(L, b, prmt); //这行等待用户输入.
  if (readstatus == 0)
...
#include "lobject.h" //需要调用什么函数就在mian里面进行测试观看.
  ...
  if (L == NULL)
  {
    ...
    return EXIT_FAILURE;
  }//lua里面调用函数都是先压入函数指针,然后压入变量.再调用pcall函数来启动压入的函数.
  ...
  status = lua_pcall(L, 2, 1, 0); /* do the call */   // 2表示2个入参, 1表示一个出参 //调用pmain函数.
